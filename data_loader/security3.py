...
class Security(Dataset):
    ...
    def __init__(self,
        ...
        data_train_x, data_train_y = su.fit_resample(data_train_x, data_train_y)
        ...
        pos_idx = np.where(self.targets==1)[0]
        neg_idx = np.where(self.targets==0)[0]
        self.idx = np.append(pos_idx, neg_idx)
        if (labeled_ratio > 0):
            ...
            #     ns = labeled_ratio
            if labeled_ratio <= 1.0:
                ns = labeled_ratio * len(neg_idx)
            else:
                ns = labeled_ratio
            ns = int(ns)
            labeled_idx = np.append(pos_idx, neg_idx[:ns])
            unlabeled_idx = neg_idx[ns:]
