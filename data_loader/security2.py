...
class Security(Dataset):
    ...
    def __init__(self,
        ...
        # downsamplng
        pos_idx = np.where(data_train_y == 1)
        neg_idx = np.where(data_train_y == 0)

        data_train_x_pos = data_train_x[pos_idx]
        data_train_x_neg = data_train_x[neg_idx]

        data_train_x_neg_down = resample(data_train_x_neg,
                                         replace=True,
                                         n_samples=2 * len(data_train_x_pos),
                                         random_state=42)
        data_train_y_pos = np.ones(len(data_train_x_pos))
        data_train_y_neg_down = np.zeros(len(data_train_x_neg_down))
        data_train_x = np.concatenate([data_train_x_pos, data_train_x_neg_down])
        data_train_y = np.concatenate([data_train_y_pos, data_train_y_neg_down])
