...
from .focalloss_bce import FocalLoss_BCE
import torch
...
class AE(BaseModel):
    def __init__(self,
        ...
        if self.mixup_method:
            ...
            # self.bce_loss = FocalLoss_BCE(alpha=0.25, gamma=2)
    ...
    def forward(self, data, device=None, **kwargs):
        ...
        lambda_indices1, lambda_indices2, one_minus_lambda_indices1, one_minus_lambda_indices2, lam1, lam2 = None, None, None, None, None, None
        ...
        if self.mixup_alpha is not None:
            ...
            lam1, lam2 = get_lambda(self.mixup_alpha, self.mixup_dist, n=mixup_n)
            if type(lam1) == float:
                lam1 = [lam1]
                lam2 = [lam2]
            lam1 = torch.from_numpy(np.array(lam1).astype('float32')).to(x.device)
            lam1 = Variable(lam1)
            lam1 = lam1.reshape(-1, 1)
            lam2 = torch.from_numpy(np.array(lam2).astype('float32')).to(x.device)
            lam2 = Variable(lam2)
            lam2 = lam2.reshape(-1, 1)
        ...
        for i, l in enumerate(self.encoder):
            ...
            if i == layer and target is not None:
                if self.mixup_within_class:
                    mixup_x, lambda_indices1, lambda_indices2, one_minus_lambda_indices1, one_minus_lambda_indices2 = mixup_class(x, target, lam1, lam2)
                else:
                    mixup_x, lambda_indices1, lambda_indices2 = mixup_process_label_free(x, lam1, lam2)
        ...
        output['lambda_indices1'] = lambda_indices1
        output['lambda_indices2'] = lambda_indices2
        output['one_minus_lambda_indices1'] = one_minus_lambda_indices1
        output['one_minus_lambda_indices2'] = one_minus_lambda_indices2
    ...
    def loss(self, output, data, device, **kwargs):
        ...
        lambda_indices1 = output['lambda_indices1']
        lambda_indices2 = output['lambda_indices2']
        one_minus_lambda_indices1 = output['one_minus_lambda_indices1']
        one_minus_lambda_indices2 = output['one_minus_lambda_indices2']
        lam1 = output['lam1']
        lam2 = output['lam2']
        ...
        if self.decoder_net and (len(self.cont_idxs)):
            ...
            if x_hat_mixup is not None:
                ...
                if self.mixup_within_class:
                    target_mixup = mixup_full_indices(x[:, self.cont_idxs], lambda_indices1, lambda_indices2, one_minus_lambda_indices1, one_minus_lambda_indices2, lam1, lam2)
                else:
                    target_mixup = mixup(x[:, self.cont_idxs], lambda_indices1, lambda_indices2, lam1, lam2)
        ...
        if self.decoder_net and (len(self.cat_idxs)):
            for i, idx in enumerate(self.cat_idxs):
                ...
                if x_hat_mixup is not None:
                    ...
                    if self.mixup_within_class:
                        target_reweighted = mixup_full_indices(target_reweighted, lambda_indices1, lambda_indices2, one_minus_lambda_indices1, one_minus_lambda_indices2, lam1, lam2)
                    else:
                        target_reweighted = mixup(target_reweighted, lambda_indices1, lambda_indices2, lam1, lam2)
        ...
        if z_proj_mixup is not None:
            if self.mixup_within_class:
                z_proj_mixup[lambda_indices1] = z_proj_mixup.clone() # - order indices
                z_proj_mixup[lambda_indices2] = z_proj_mixup.clone()
                ...
                if weights is not None:
                    weights_mixup = mixup_full_indices(weights, lambda_indices1, lambda_indices2, one_minus_lambda_indices1, one_minus_lambda_indices2, lam1, lam2)
                    weights_mixup[lambda_indices1] = weights_mixup.clone()
                    weights_mixup[lambda_indices2] = weights_mixup.clone()
                ...
                mask[torch.arange(z_proj.size(0)), torch.arange(z_proj.size(0))] = lam1.squeeze().cpu()
                mask[torch.arange(z_proj.size(0)), lambda_indices1] = lam2.squeeze().cpu()
                mask[torch.arange(z_proj.size(0)), lambda_indices2] = 1 - lam1.squeeze().cpu() - lam2.squeeze().cpu()
