...
class MLP(BaseModel):
    def __init__(self,
        ...
        # self.criterion_mixup = FocalLoss_BCE(alpha=0.25, gamma=2)
        self.criterion_mixup = nn.BCELoss(reduction='none')
        # self.criterion_softmax = FocalLoss(gamma=2, alpha=0.25)
        # samples_per_cls = torch.tensor([4510, 990])
        # self.criterion_softmax = CB_loss(beta=0.9999, gamma=2.0, loss_type='focal', no_of_classes=num_classes, samples_per_cls=samples_per_cls)
    ...
    def forward(self, data, device, **kwargs):
        ...
        for i, l in enumerate(self.predictor):
            ...
            if i == layer and target_reweighted is not None:
                x, target_reweighted, indices1, indices2 = mixup_process(x, target_reweighted, lam1, lam2)

            if (i == layer) and (x_u is not None) and (utarget_reweighted is not None) and (self.mixup_method != ''):
                x_u, utarget_reweighted, uindices1, uindices2 = mixup_process(x_u, utarget_reweighted, lam1, lam2)
        ...
        out['lam1'] = lam1
        out['lam2'] = lam2
        out['uindices1'] = uindices1
        out['uindices2'] = uindices2
        out['indices1'] = indices1
        out['indices2'] = indices2
